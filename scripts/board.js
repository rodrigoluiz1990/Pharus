...
const BoardModule = (() => {
    ...
    const createTaskElement = (task, users) => {
        ...
        taskElement.innerHTML = `
            ...
            <div>
                <span class="task-tag tag-status">${UtilsModule.getStatusText(task.status)}</span>
                <span class="task-tag ${priorityClass}">${UtilsModule.getPriorityText(task.priority)}</span>
            </div>
            <div class="task-meta">
                <span>${assignee ? assignee.name : 'Não atribuído'}</span>
