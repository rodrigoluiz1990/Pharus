...
const SettingsPermissionsModule = (() => {
    ...
    const ensureSchema = async () => {
        try {
            const { error } = await window.supabaseClient.from('permission_groups').select('id').order('name', { ascending: true });
    ...
    const loadGroups = async () => {
        ...
        setStatus(groupsStatusEl, groups.length ? `${groups.length} grupo(s) cadastrado(s).` : 'Nenhum grupo cadastrado.');
    };
    ...
    const saveGroup = async () => {
        ...
        if (selectedGroupId) {
            const { error } = await window.supabaseClient.from('permission_groups').update({ name, description: description || null, status }).eq('id', selectedGroupId);
            if (error) throw error;
        } else {
            const { data, error } = await window.supabaseClient
                .from('permission_groups')
    ...
    const deleteGroup = async () => {
        ...
        const { error } = await window.supabaseClient.from('permission_groups').delete().eq('id', group.id);
        if (error) throw error;
    ...
    const init = async () => {
        if (!groupsListEl || !matrixGridEl || !window.supabaseClient) return;
        renderMatrix();
