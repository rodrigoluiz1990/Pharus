...
const RelatoriosModule = (() => {
    ...
    const DEFAULT_DEFINITION = {
        columns: ['title', 'status', 'priority', 'assignee', 'client', 'due_date'],
    ...
    const priorityLabel = (priority) => ({ low: 'Baixa', medium: 'Média', high: 'Alta' }[String(priority || '')] || String(priority || '-'));
    const typeLabel = (type) => ({ task: 'Novo', bug: 'Erro', improvement: 'Melhoria' }[String(type || '')] || String(type || '-'));
    ...
    const normalizeDefinition = (rawDefinition) => {
        ...
        return {
            ...
            filters: {
                ...
                priority: String(input?.filters?.priority || ''),
                type: String(input?.filters?.type || ''),
    ...
    const loadGroups = async () => {
        const { data, error } = await window.supabaseClient.from('permission_groups').select('id,name,status').order('name', { ascending: true });
    ...
    const loadShares = async () => {
        const { data, error } = await window.supabaseClient.from('task_report_group_shares').select('*');
        if (error) throw error;
    ...
    const applyTaskFilters = (tasks, filters) => {
        ...
        return (Array.isArray(tasks) ? tasks : []).filter((task) => {
            ...
            if (type && String(task.type || '') !== type) return false;
...
<script>window.onload=function(){window.print();}</script></body></html>`;
    ...
    const syncReportShares = async (reportId, selectedGroupIds) => {
        await window.supabaseClient.from('task_report_group_shares').delete().eq('report_id', reportId);
        if (!selectedGroupIds.length) return;
        const rows = selectedGroupIds.map((groupId) => ({ report_id: reportId, group_id: groupId, created_by: currentUserId }));
        const { error } = await window.supabaseClient.from('task_report_group_shares').insert(rows);
        if (error) throw error;
    };
