...
const SortModule = (() => {
    ...
    const PRIORITY_WEIGHT = { very_low: 1, low: 2, medium: 3, high: 4, very_high: 5 };
    ...
    const parseFieldValue = (task, field) => {
        if (field === 'priority') {
            const normalized = (window.UtilsModule && typeof window.UtilsModule.normalizePriorityKey === 'function')
                ? window.UtilsModule.normalizePriorityKey(task.priority)
                : String(task.priority || '');
            return PRIORITY_WEIGHT[normalized] || 0;
        }
    ...
    const getRawFieldValue = (task, field) => {
        if (field === 'priority' && window.UtilsModule && typeof window.UtilsModule.normalizePriorityKey === 'function') {
            return window.UtilsModule.normalizePriorityKey(task?.priority || '');
        }
        if (field === 'type' && window.UtilsModule && typeof window.UtilsModule.normalizeTypeKey === 'function') {
            return window.UtilsModule.normalizeTypeKey(task?.type || '');
        }
    ...
    const getManualCriterionFromLegacy = (field, optionOrder) => {
        ...
        if (field === 'type') {
            if (optionOrder === 'task_bug_improvement') return { field, mode: 'manual', order: ['new', 'issue', 'improvement'] };
            if (optionOrder === 'bug_first') return { field, mode: 'manual', order: ['issue', 'new', 'improvement'] };
        }
