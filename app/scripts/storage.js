const StorageModule = (() => {
    ...
    const parseDateOnlyToIsoRange = (dateValue) => {
        ...
        const ymdDateTimeMatch = raw.match(/^(\d{4})-(\d{2})-(\d{2})T/);
        ...
        } else if (ymdDateTimeMatch) {
            year = Number(ymdDateTimeMatch[1]);
            month = Number(ymdDateTimeMatch[2]);
            day = Number(ymdDateTimeMatch[3]);
        } else if (dmyMatch) {
    ...
    const getCurrentUserId = async () => {
        try {
            ...
            const { data: mappedUser, error: mappedError } = await dbClient
                .from('app_users')
                .select('id,email')
                .eq('email', email)
                .single();
            if (mappedError || !mappedUser) return null;

            const mappedId = String(mappedUser.id || '').trim();
    ...
    const syncTaskToAgenda = async (taskRecord) => {
        ...
        let dueDate = String(taskRecord?.due_date || '').trim();
        if (!dueDate && taskRecord?.delivery_date) {
            dueDate = String(taskRecord.delivery_date).trim();
        }
    ...
    // REMOVER ESTA FUNÇÃO - usar auth do dbClient em vez disso
    ...
    const getTasks = async () => {
        try {
            ...
            const { data: tasksData, error: tasksError } = await dbClient
                .from('tasks')
                .select('*')
                .order('created_at', { ascending: false });
