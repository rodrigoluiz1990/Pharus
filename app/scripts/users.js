...
const UsersModule = (() => {
    ...
    const loadUsers = async () => {
        try {
            ...
            const { data: { session }, error: sessionError } = await window.dbClient.auth.getSession();
    ...
    const loadCurrentUserPermissions = async () => {
        ...
        if (!window.dbClient || !currentUser?.id) return;
        ...
        try {
            ...
            const { data: ruleRows, error: ruleError } = await window.dbClient
    ...
    const loadPermissionGroups = async () => {
        if (!window.dbClient || !userPermissionGroup) return;
        try {
    ...
    const loadUsersFromView = async () => {
        try {
            const { data: usersData, error } = await window.dbClient
                .from('app_users')
        ...
        try {
            ...
            if (!canEditTarget) {
                ...
                if (typeof UtilsModule.showPermissionDeniedModal === 'function') {
                    UtilsModule.showPermissionDeniedModal('Você não tem permissão para editar outros usuários.');
                } else {
            ...
            if (isCurrentUser) {
                ...
                const { error: authError } = await window.dbClient.auth.updateUser(updatePayload);
                
                if (authError) throw authError;

                const { error: profileError } = await window.dbClient
                    .from('app_users')
                    .update({
                        role: userData.role,
                        status: userData.status,
    ...
    const openEditUserModal = async (userId) => {
        ...
        if (!canEditTarget) {
            if (typeof UtilsModule.showPermissionDeniedModal === 'function') {
                UtilsModule.showPermissionDeniedModal('Apenas usuários com permissão de editar usuários podem alterar outros perfis.');
            } else {
