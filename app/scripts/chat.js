...
const ChatModule = (() => {
    ...
    const loadCurrentUser = async () => {
        try {
            ...
            if (!window.dbClient.auth || typeof window.dbClient.auth.getUser !== 'function') {
                console.warn('Método auth.getUser não disponível');
                throw new Error('Autenticação não disponível');
            }
            
            const { data, error } = await window.dbClient.auth.getUser();
    ...
    const tryGetUserFromSession = async () => {
        try {
            if (window.dbClient && window.dbClient.auth.getSession) {
    ...
    const loadMessages = async (contactId, options = {}) => {
        ...
        try {
            ...
            const { data: messages, error } = await window.dbClient
                .from('chat_messages')
                .select('*')
    ...
    const setupRealtimeSubscription = () => {
        ...
        if (chatSocket) {
            window.dbClient.removeChannel(chatSocket);
        }
        
        chatSocket = window.dbClient
    ...
    const fetchContactsWithPresence = async () => {
        let contactsResult = await window.dbClient
            .from('user_profiles')
        ...
        contactsResult = await window.dbClient
            .from('user_profiles')
    ...
    const pingPresence = async () => {
        if (!currentUser?.id || !window.dbClient) return;
        try {
